...
import {
  ...
  provider,
  signInWithPopup,
} from "src/firebase/firebaseConfigs";
...
export default function ConversationScreen(props: any) {
  ...
  function getMessages(conversationId: string, callback: any) {
    return onSnapshot(
      query(
  ...
  return (
    <StyledMessageContainer>
      <StyledHeader>
        {" "}
        Avatar and name header of conversation
        <div>{/* {"Avatar and name of user"} */}</div>
        <div>
      ...
      >
        {messages.map((message: any, index) => (
          <ListItem alignItems="flex-start" key={index}>
            ...
            />
            <div style={{ marginBottom: "2px" }} ref={endOfMessagesRef}>
              {" "}
            </div>
          </ListItem>
        ))}
